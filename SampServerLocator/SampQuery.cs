...
namespace SampQueryApi
{
    sealed class SampQuery
    {
        ...
        private byte[] SendSocketToServer(in char packetType)
        {
            ...
            {
                var bye = new byte[szReceiveArraySize];
                //MessageBox.Show(e.Message, "Connection Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return bye;
            }
        }
        ...
        public SampServerRulesData GetServerRules()
        {
            ...
            for (int i = 0, iRules = read.ReadInt16(); i < iRules; i++)
            {
                ...
                if (property != null)
                {
                    ...
                    {
                        DateTime dateTimeget = DateTime.Now;
                        try
                        {
                            val = dateTimeget.Add(TimeSpan.Parse(value));
                        }
                        catch (Exception)
                        {
